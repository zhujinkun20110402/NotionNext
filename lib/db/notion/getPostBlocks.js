...
export async function fetchNotionPageBlocks(id, from = null, slice = 0) {
  ...
  // 3️⃣ 过滤掉无效的block（id为undefined的block）
  if (pageBlock?.recordMap?.block) {
    Object.keys(pageBlock.recordMap.block).forEach(key => {
      const block = pageBlock.recordMap.block[key]
      if (!block?.value?.id) {
        console.warn(`[过滤无效block] 移除id为undefined的block: ${key}`)
        delete pageBlock.recordMap.block[key]
      }
    })
  }
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    console.log('[API<<--响应]')
    const newBlocks = pageChunk?.recordMap?.block || {}
    // 过滤无效block
