 ...
 * @returns
 */
const LayoutNoteTimeline = props => {
  const { notePosts } = props

  return (
    <div className='p-5 rounded-xl border dark:border-gray-600 max-w-4xl w-full bg-white dark:bg-[#1e1e1e] mx-auto'>
      {/* 页面标题 */}
      <div className='mb-8 text-center'>
        <h1 className='text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2'>琐记</h1>
        <p className='text-gray-600 dark:text-gray-400'>记录日常点滴，时光如梭</p>
      </div>
      ...
      <div className='px-3'>
        {Object.keys(notePosts).map(noteDate => (
          <NoteTimeline
            key={noteDate}
            posts={notePosts[noteDate]}
            archiveTitle={noteDate}
